...
{
    // Convert exotic:// to exotic:
    //
    //    Cannot handle this later, because exotic:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open exotic.conf with the associated application */
...
{
    return GetAutostartDir() / "exotic.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a exotic.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the exotic app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add exotic app to startup item list
