...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GALILELMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GALILELMiner : ProcessNewBlock, block not accepted");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("GALILELMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running GALILELMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadGALILELMiner() exception");
    } catch (...) {
        LogPrintf("ThreadGALILELMiner() exception");
    }

    LogPrintf("ThreadGALILELMiner exiting\n");
}
